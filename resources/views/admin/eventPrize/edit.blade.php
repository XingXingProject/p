...
@section('content')
    <form class="form-horizontal" method="post" enctype="multipart/form-data">
            ...
            <div class="col-sm-6">
                分类：<select name="event_id">
                    ...
                    @foreach($actives as $active)
                        <option value="{{$active->id}} "@if($event->id===$active->id) selected @endif>{{$active->title}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <input type="text" class="form-control" id="inputEmail3" placeholder="" name="name" value="{{old("name",$event->name)}}">
            </div>
        </div>
